...
from ocr_wrapper.azure import _discretize_angle_to_90_deg, _determine_endpoint_and_key
import pytest
...
def test_azure_ocr(ocr):
    ...
    # Check if every entry has a confidence
    for r in res:
...
def test_azure_ocr_auto_rotation(unrotated_bboxes, ocr_with_auto_rotate):
    ...
    for img_filename in rotated_images_list:
        ...
        for unrot_bbox, rot_bbox in zip(unrotated_bboxes, rotated_bboxes):
            assert unrot_bbox["bbox"].coords == pytest.approx(rot_bbox["bbox"].coords, abs=0.1)
